...
function App() {
  ...
  const stepLabels = ['Home', 'Story', 'Timeline', 'RSVP', 'Gallery', 'Travel', 'Gifts', 'Share the joy']
  ...
  return (
    <div className={`page-shell has-progress ${isStepper ? 'is-stepper' : ''}`}>
        ...
        <header className="hero-section" id="home">
          ...
          <nav className="top-nav">
            ...
            <div className="nav-links">
              {!isStepper && config.navItems.map((item) => (
          ...
          <div className="hero-grid">
            <div className="hero-copy">
              ...
              <p className="hero-lead">
                {config.couple.brideFull} and {config.couple.groomFull} joyfully invite you
                to share in their wedding day — a celebration of culture, faith,
                and the beautiful love story that began in 2021.
              </p>

              <div className="hero-actions">
                <a className="button button-primary" href="#rsvp" onClick={isStepper ? (e) => { e.preventDefault(); handleStepChange(3); } : undefined}>
                  Confirm attendance
                </a>
                <a className="button button-secondary" href="#travel" onClick={isStepper ? (e) => { e.preventDefault(); handleStepChange(5); } : undefined}>
                  View venue and map
                </a>
              </div>
            ...
            <aside className="countdown-card">
              ...
              <p className="countdown-note">
                {config.timeline[0].title} begins at {config.timeline[0].time}, followed by {config.timeline[1].title} at
                {config.timeline[1].time} and {config.timeline[2].title} at {config.timeline[2].time}.
              </p>
            </aside>
          </div>
          {renderNavigation(0)}
        ...
        <>
          <section className="facts-strip reveal">
            {config.weddingFacts.map((fact, i) => (
              <article className="fact-card" key={fact.label} style={{ animationDelay: `${i * 0.1}s` }}>
      ...
      {renderSection(1, (
        <section className="section two-column-section reveal" id="story">
          <div className="section-heading">
            ...
            <p>
              From the day we met, our journey has been one of faith,
          ...
          <div className="story-grid">
            {config.storyMoments.map((moment, i) => (
              <article className="glass-card" key={moment.title} style={{ animationDelay: `${i * 0.2}s` }}>
                <h3>{moment.title}</h3>
                <p>{moment.body}</p>
              </article>
            ))}
        ...
        <section className="section reveal" id="schedule">
          ...
          <div className="timeline">
            {config.timeline.map((item, i) => (
              <article className="timeline-item" key={item.time} style={{ animationDelay: `${i * 0.2}s` }}>
      ...
      {renderSection(3, (
        <section className="section rsvp-section reveal" id="rsvp">
          <div className="section-heading">
            ...
            <p>
              Share your details so we can prepare for you and send your access card via WhatsApp.
            </p>
          </div>
          ...
          {!isStepper ? (
            <div className="faq-list">
              {config.faqItems.map((item, i) => (
                <article className="faq-card" key={item.title} style={{ animationDelay: `${i * 0.2}s` }}>
                  <h3>{item.title}</h3>
                  <p>{item.detail}</p>
                </article>
              ))}
          ...
          <form className="rsvp-form glass-card rsvp-card" onSubmit={handleSubmit}>
      ...
      {!isStepper && (
        <section className="parallax-banner parallax-banner-gallery reveal">
          <div className="parallax-content">
      ...
      {renderSection(4, (
        <section className="section reveal" id="gallery">
          <div className="section-heading">
            ...
            <p>
              A glimpse of the journey so far. Pre-wedding photos haven't been taken yet — introduction pictures will be featured here soon.
            </p>
          </div>

          <div className="gallery-grid">
            {config.galleryItems.map((item, i) => (
              <article className="gallery-card" key={item.title} style={{ animationDelay: `${i * 0.15}s` }}>
        ...
        <section className="section split-section reveal" id="travel">
          <div>
            <div className="section-heading section-heading-left">
              ...
              <p>
                All ceremonies will take place at {config.theme.venue}.
              </p>
            </div>

            <div className="venue-grid">
              {config.venueLinks.map((venue, i) => (
                <article className="venue-card" key={venue.label} style={{ animationDelay: `${i * 0.2}s` }}>
                  <h3>{venue.label}</h3>
                  <p>{venue.address}</p>
              ...
              <div className="color-chips">
                {parseThemeColors(config.theme.colors).map((name) => (
                  <span className="color-chip" key={name}>
                    <span className="color-dot" style={{ background: colorToHex(name) }}></span>
                    {name}
                  </span>
                ))}
              </div>
          ...
          <aside className="travel-notes glass-card">
            ...
            <p>
              For directions, access cards, or anything you need on the day, kindly reach our wedding coordinator:
            </p>
            <p>
              <strong>{config.socialLinks.find((l) => l.label.includes('Coordinator'))?.label.split(': ')[1] || '+234 808 298 6043'}</strong>
            </p>
            <p>
              After you RSVP, your access card will be sent to your WhatsApp number ahead of the celebration.
            </p>
          </aside>
          {renderNavigation(5)}
        </section>
        ...
        <section className="section gifts-section reveal" id="gifts">
          ...
          <div className="gifts-grid">
            <article className="glass-card gift-highlight">
              ...
              <div className="gift-account">{config.registryOptions[0]}</div>
              <p className="gift-note">Thank you for your love, support, and generosity.</p>
            </article>
        ...
        <section className="section social-section reveal" id="social">
          ...
          <div className="social-links">
            {config.socialLinks.map((link, i) => (
              <a key={link.label} href={link.href} target="_blank" rel="noreferrer" style={{ animationDelay: `${i * 0.1}s` }}>
                {link.label}
              </a>
            ))}
          </div>
      ...
      {!isStepper || currentStep === 7 ? (
        <footer className="site-footer">
          <p>With love, {config.couple.brideFull} and {config.couple.groomFull}</p>
          <p>{config.weddingDateFormatted} · {config.theme.venue} · {config.theme.colors}</p>
        </footer>
      ) : null}
    </div>
  )
